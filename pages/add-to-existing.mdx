...
<Tabs id="entry" items={['React',  'Solid', 'Vue', 'Svelte', 'Vanilla']} storageKey="tabs_existing">
  <Tabs.Tab>
    ```ts copy
    // add to ./src/main.tsx
...
<Tabs items={['React',  'Solid', 'Vue', 'Svelte', 'Vanilla']} storageKey="tabs_existing">
<Tabs.Tab>
  ```tsx copy {2,6-8,12}
  //./src/App.tsx
  import {r} from "./main.jsx";
...
<Tabs.Tab>
  ...
  export function setupCounter(element: HTMLButtonElement) {
    ...
    r.subscribe(
      (tx) => tx.get("count"),
      (count) => (element.textContent = (count ?? 0).toString())
    );
  }
  ```
</Tabs.Tab>
