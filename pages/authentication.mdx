...
The more common implementation is to call an endpoint on your server that validates the `authToken` and returns the `UserData` for the user. Something like:
...
```ts
async function jwtAuthHandler(auth: string, roomID: string): Promise<UserData> {
...
However, it is also possible to control whether each individual mutation is allowed. To do so, return additional data as part of the `UserData` object. This data will be passed to mutators via the `WriteTransaction.auth` parameter. You can then use this data to decide whether to allow the mutation:

```ts
type MyUserData = UserData & { access: "read" | "write" };
...
async function detailedAuthHandler(
  ...
  const response = await fetch(`/reflect-auth?auth=${auth}&roomID=${roomID}`);
  return (await response.json()) as MyUserData;
}
```
...
const mutators = {
  editNodeContent: async (tx: WriteTransaction,
    ...
    const auth = tx.auth as MyUserData;
