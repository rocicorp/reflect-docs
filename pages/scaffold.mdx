...
Reflect has a local dev mode for fast iteration. The `watch` script inside this starter app runs both the Reflect dev server (`npx reflect dev`) and the Vite UI server (`npx vite`):
